...
import { Link, UserProfile, LinkType, PromoData } from '../types';
...
const Dashboard: React.FC = () => {
  ...
  const filteredLinks = links.filter(l => showPromoHistory || l.origin !== 'promo');
      ...
      <div className="flex-1 space-y-6 animate-in fade-in duration-500">
        <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200">
          <div className="flex flex-col md:flex-row items-start gap-8">
            ...
            <div className="flex-1 w-full space-y-4">
              <input value={profile.displayName} onChange={(e) => setProfile({...profile, displayName: e.target.value})} className="text-3xl font-black text-slate-900 bg-transparent border-none p-0 focus:ring-0 w-full" placeholder="Your Name" />
              <div className="relative">
                <textarea value={profile.bio} onChange={(e) => setProfile({...profile, bio: e.target.value})} className="w-full text-slate-700 font-medium text-base h-24 resize-none bg-slate-50 border border-slate-200 rounded-3xl p-5 outline-none focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all" placeholder="Share your story..." />
                <button onClick={async () => { setIsOptimizing(true); const newBio = await optimizeBio(profile.bio, profile.displayName); setProfile({...profile, bio: newBio}); setIsOptimizing(false); }} disabled={isOptimizing} className="absolute bottom-4 right-4 bg-indigo-600 text-white px-5 py-2 rounded-2xl text-xs font-bold hover:bg-indigo-700 transition-all flex items-center gap-2 shadow-lg shadow-indigo-100 disabled:opacity-50">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[
            ...
            <button key={btn.id} onClick={() => addLink(btn.id as LinkType)} className={`${btn.color} text-white py-4 rounded-[1.5rem] font-bold shadow-lg hover:-translate-y-1 active:scale-95 transition-all flex flex-col items-center justify-center gap-2`}>
        ...
        <div className="space-y-4">
          {filteredLinks.length === 0 ? (
            ...
            return (
              <div key={link.id} className={`bg-white p-6 rounded-[2rem] border-2 flex gap-6 group transition-all relative ${link.isHeroVideo ? 'border-amber-100 bg-amber-50/20 shadow-inner' : 'border-slate-100 hover:border-indigo-100'}`}>
                {link.origin === 'promo' && (
                ...
                <div className="flex-1 space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 w-full">
                      ...
                      <input value={link.title} onChange={(e) => updateLink(link.id, { title: e.target.value })} className="text-xl font-black text-slate-900 bg-transparent border-none focus:ring-0 w-full" placeholder="Title" />
                      <button onClick={() => handleRewriteTitle(link.id, link.title)} disabled={rewritingId === link.id} className="text-indigo-600 p-2 hover:bg-indigo-100 rounded-lg transition-colors"><i className={`fa-solid ${rewritingId === link.id ? 'fa-spinner fa-spin' : 'fa-sparkles'}`}></i></button>
                    </div>
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-3 bg-white/50 border border-slate-200 rounded-2xl px-5 py-3 shadow-sm">
                      ...
                      <input value={link.url} onChange={(e) => updateLink(link.id, { url: e.target.value })} className="bg-transparent text-sm font-bold text-slate-800 outline-none w-full" placeholder="Destination URL" />
                    </div>
                    {link.isHeroVideo && ytId && (
                      <div className="relative w-full aspect-video bg-zinc-900 rounded-2xl overflow-hidden group/hero border border-amber-200/50 shadow-inner">
                        <iframe src={`https://www.youtube.com/embed/${ytId}?autoplay=0&mute=1&controls=0`} className={`w-full h-full pointer-events-none transition-all ${link.isNSFW ? 'blur-2xl opacity-40 scale-110' : 'opacity-100'}`} title="Preview"></iframe>
                      </div>
                    )}
                  </div>
                  <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                     <div className="flex flex-wrap items-center gap-6 text-[11px] font-bold uppercase tracking-widest text-slate-500">
                        ...
                        <button onClick={() => updateLink(link.id, { isHeroVideo: !link.isHeroVideo })} className={`flex items-center gap-2 transition-all p-2 rounded-xl ${link.isHeroVideo ? 'text-amber-600 bg-amber-50' : 'hover:text-slate-900 hover:bg-slate-50'}`}>
                        ...
                        <button onClick={() => updateLink(link.id, { isNSFW: !link.isNSFW })} className={`flex items-center gap-2 transition-colors p-2 rounded-xl ${link.isNSFW ? 'text-red-500 bg-red-50' : 'hover:text-slate-900 hover:bg-slate-50'}`}>
